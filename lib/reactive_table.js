var ReactiveTable2Counts = new Mongo.Collection("reactive-table-counts-2");
...
var updateHandle = function (set_context) {
    ...
    if (context.server) {
        ...
        var newPublishedRows = new Mongo.Collection('reactive-table-rows-2-' + publicationId);
        ...
        var onError = function (error) {
            console.log("ReactiveTable2 subscription error: " + error);
        };
...
var setup = function () {
    ...
    if (hasChildren) {
        ...
        context.childrenExpandIconClass = this.data.settings.childrenExpandIconClass || 'reactive-table-2-children-expand';
...
var getRowCount = function () {
    if (this.server) {
        var count = ReactiveTable2Counts.findOne(this.publicationId.get());
...
Template.reactiveTable2.onCreated(function() {
...
Template.reactiveTable2.onDestroyed(function() {
  if (this.context.server && this.context.handle) {
    this.context.handle.stop();
  }
});

Template.reactiveTable2.helpers({
    'context': function () {
    ...
    getTest: function(){debugger}
});

Template.reactiveTable2.events({
    'click .reactive-table-2 .sortable': function (event) {
        var template = Template.instance();
    ...
    'click .reactive-table-2 > tbody > tr > td': function (event) {

        // TODO: there is a bug if you call _collapseChildren elsewhere, the icons need to update
        if (!this.tmpl){

            var template = Template.instance();
            var iconExpand = $(event.currentTarget).find('.' + template.context.childrenExpandIconClass);
            ...
            if (template.context.useFontAwesome){
                ...
                if (iconExpand.hasClass( 'fa-plus-square-o' )){
                    iconExpand.removeClass( 'fa-plus-square-o' ).addClass( 'fa-minus-square-o' );
                }
                else if (iconExpand.hasClass( 'fa-minus-square-o' )){
