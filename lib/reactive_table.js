...
if (Handlebars) {
    Handlebars.registerHelper('reactiveTable', function (collection, settings) {
        ...
        Session.setDefault(getSessionSortKey(group), fields[0].key || fields[0]);
        Session.setDefault(getSessionSortDirectionKey(group), 1);
        Session.setDefault(getSessionRowsPerPageKey(group), 10);
...
Template.reactiveTable.helpers({
    ...
    "isSortKey": function (field, group) {
        return Session.equals(getSessionSortKey(group), field.key || field);
    },
    ...
    "sortedRows": function () {
        var sortKey = Session.get(getSessionSortKey(this.group));
...
Template.reactiveTable.events({
    "click .reactive-table .sortable": function (event) {
        ...
        var currentSortKey = Session.get(getSessionSortKey(group));
        if (currentSortKey === sortKey) {
            ...
            Session.set(getSessionSortKey(group), sortKey);
        }
    },
