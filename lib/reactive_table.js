...
var setup = function () {
    ...
    // TODO: add option for a colSpan container?
...
Template.reactiveTable.helpers({
    ...
    'getChildrenRows': function() {
        var childrenRows = this[ Template.parentData().children.key ];
    ...
    'addChildrenExpandIcon': function() {
        ...
        if (context.children != null) {
            if (context.children.expandFieldId != null) {
                if (context.children.expandFieldId === this.fieldId) {
                    return true;
                }
            }
            else if (context.children.key != null && context.children.key === this.key) {
                return true;
            }
        }
    },
...
Template.reactiveTable.events({
    ...
    'click .reactive-table > tbody > tr > td': function (event) {
        ...
        var childrenExpandIconClass = template.context.childrenExpandIconClass;

        if (childrenExpandIconClass == null) {
            return
        }

        var iconExpand = $(event.currentTarget).find('.' + childrenExpandIconClass);
        ...
        // TODO: if it's a template we need to defer to the tmpl helper
        if (this.tmpl != null) {
