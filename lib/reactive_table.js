...
var getRowCount = function () {
    if (this.server) {
        return ReactiveTableCounts.findOne(this.publicationId.get());
    } else {
...
Template.reactiveTable.helpers({
    ...
    'showRowCount': function() {
        if(this.showRowCount)
            var rowCount = getRowCount.call(this);
            if (_.isObject(rowCount)) return rowCount.count;
            else if (rowCount) return rowCount;
            return '0';
    }
});
