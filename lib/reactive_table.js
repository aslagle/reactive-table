...
var setup = function () {
    ...
    context.childrenExpandLabel = this.data.settings.childrenExpandLabel || null;
    context.childrenKey = this.data.settings.childrenKey || null;
    context.childrenFields = this.data.settings.childrenFields || null;
...
Template.reactiveTable.helpers({
    ...
    'getChildrenRows': function(){
        var childrenRows = this[ Template.parentData().childrenKey ];
    ...
    'addChildrenExpandIcon': function(){
        var context = Template.instance().context;
        if (context.childrenExpandLabel != null){
            if (context.childrenExpandLabel === this.label){
                return true;
            }
        }
        else if (context.childrenKey != null && context.childrenKey == this.key){
            return true;
        }
    },
...
Template.reactiveTable.events({
    ...
    'click .reactive-table-children-expand': function( event ){
        var iconExpand = $(event.target);
        var nextRow = iconExpand.closest( 'tr' ).next( '.reactive-table-children-container' );

        if (Template.instance().context.useFontAwesome){
