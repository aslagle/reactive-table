...
var setup = function () {
    ...
    context.showRowCount = this.data.showRowCount || this.data.settings.showRowCount || true;
...
var getRowCount = function (filterQuery) {
  if(this.isServer) {
      return {
        totalRows: ReactiveTableCounts.findOne(this.publicationId.get()),
        totalFilteredRows: ''
      };
  } else {
      ...
      return {
        totalRows: this.collection.find().count(),
        totalFilteredRows: this.collection.find(filterQuery).count()
      };
...
var getPageCount = function () {
    ...
    if (this.server) {
        count = getRowCount.call(this).totalRows;
        return Math.ceil((count ? count.count : 0) / rowsPerPage);
    } else {
        count = getRowCount.call(this).totalFilteredRows;
