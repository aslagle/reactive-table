...
var getSessionPageCount = function (group) {
    return group + '-reactive-table-page-count';
};
...
Template.reactiveTable.getPageCount = function () {
    ...
    Session.set(getSessionPageCount(this.group), pageCount);
    return pageCount;
};
...
Template.reactiveTable.helpers({
    ...
    'getCurrentPage' : function () {
        var pageCount = Session.get(getSessionPageCount(this.group));
        var currentPage = 1 + Session.get(getSessionCurrentPageKey(this.group));
        if (currentPage > pageCount) {
          currentPage = pageCount - 1;
          Session.set(getSessionCurrentPageKey(this.group), currentPage);
        }
        if (currentPage < 0) {
          currentPage = 1;
          Session.set(getSessionCurrentPageKey(this.group), currentPage);
...
Template.reactiveTable.events({
    ...
    'change .reactive-table-navigation .rows-per-page input': function (event) {
        try {
            var rowsPerPage = Math.max(~~parseInt($(event.target).val(), 10), 1);
    ...
    'change .reactive-table-navigation .page-number input': function (event) {
        try {
            var currentPage = Math.max(~~parseInt($(event.target).val(), 10) - 1, 1);
            var group = $(event.target).parents('.reactive-table-navigation').attr('reactive-table-group');
