...
Template.reactiveTable.events({
    ...
    'click .reactive-table-export': function (event) {
        var template = Template.instance();
        var rows = _.map(sortedRows.call(template.context).fetch(), function (row) {
            delete row['reactive-table-id'];
            delete row['reactive-table-sort'];
            return row;
        });
        //var json = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(rows));
        var fields = template.context.fields;
        var headerRow = "\n" + _.map(fields, function (field) {
            return field.label;
        }).join(",") + "\n";
        var csvData = "data:text/csv;charset=utf-8," + encodeURI(headerRow);
        _.each(rows, function (row) {
            csvData += encodeURI(_.map(fields, function (field) {
                return getField.call(field, row);
            }).join(",") + "\n");
        });
        console.log(csvData);
        window.open(csvData);
