...
ReactiveTable.publish = function (name, collection, selector) {
  if (collection) {
    Meteor.publish("reactive-table-" + name, function (publicationId, filter, fields, options, rowsPerPage) {
      ...
      var self = this;
      var filterQuery = _.extend(getFilterQuery(filter, fields), selector);
        ...
        _.each(newRows, function (row, index) {
          ...
          if (oldRow) {
            if (!_.isEqual(oldRow, row)) {
              self.changed("reactive-table-rows", row._id, row);
              rows[row._id] = row;
            }
          } else {
            self.added("reactive-table-rows", row._id, row);
            rows[row._id] = row;
          }
        });
      };
      ...
      var handle = cursor.observeChanges({
        ...
        removed: function (id, fields) {
          self.changed("reactive-table-counts", publicationId, {count: cursor.count()});
          self.removed("reactive-table-rows", id, getRow(fields));
          delete rows[id];
          updateRows();
        },
