...
ReactiveTable2.publish = function (name, collectionOrFunction, selectorOrFunction, settings) {
    Meteor.publish("reactive-table-" + name, function (publicationId, filters, fields, options, rowsPerPage) {
      ...
      if (!(collection instanceof Mongo.Collection)) {
        console.log("ReactiveTable2.publish: no collection to publish");
        return [];
      }
        ...
        _.each(newRows, function (row, index) {
          ...
          if (oldRow) {
            if (!_.isEqual(oldRow, row)) {
              self.changed("reactive-table-rows-2-" + publicationId, row._id, row);
              rows[row._id] = row;
            }
          } else {
            self.added("reactive-table-rows-2-" + publicationId, row._id, row);
            rows[row._id] = row;
          }
        });
      };

      self.added("reactive-table-counts-2", publicationId, {count: count});
      ...
      var handle = pageCursor.observeChanges({
        ...
        removed: function (id, fields) {
          self.removed("reactive-table-rows-2-" + publicationId, id);
          delete rows[id];
          updateRows();
        },
      ...
      var countHandle = fullCursor.observeChanges({
          added: function (id, fields) {
              if (!initializing) {
                  self.changed("reactive-table-counts-2", publicationId, {count: fullCursor.count()});
              }
          },
