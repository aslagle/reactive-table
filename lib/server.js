...
ReactiveTable.publishComposite = function (name, collectionOrFunction, selectorOrFunction, settings) {
  Meteor.publishComposite("reactive-table-" + name, function (publicationId, filters, fields, options, rowsPerPage) {
    ...
    return {
      find: function() {
        console.log(check)
        console.log(publicationId, filters, fields, options, rowsPerPage)
        check(publicationId, String);
        check(filters, [Match.OneOf(String, Object, Mongo.ObjectID)]);
        check(fields, [[String]]);
        check(options, {skip: Match.Integer, limit: Match.Integer, sort: Object});
        ...
        var self = this;
        var filterQuery = _.extend(getFilterQuery(filters, fields, settings), selector.find);
        if (settings && settings.fields) {
          options.fields = settings.fields;
        }
        var pageCursor = collection.find(filterQuery, options);
